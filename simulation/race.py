...
# A voluntary stop must beat staying out by at least this many seconds to be
# worth it (defined in simulation/ai/heuristic.py alongside the strategy logic).
...
# AI weather-swap thresholds now live in simulation/ai/heuristic.py alongside
# the moved strategy logic.
...
# Tactical pit behaviour (undercut/cover/Safety-Car thresholds) now lives in
# simulation/ai/heuristic.py alongside the strategy logic.
...
def create_race(drivers, circuit, player_team, grid=None, starting_tyres=None, track_wetness=None, rng=None):
    ...
    return {
        ...
        # Pluggable AI engine name (plain string, save-safe). Only ever
        # "heuristic" or "api" — never an engine object or an API key.
        "ai_engine": "heuristic",
        "rng": rng,
    }
...
def run_lap(race_state, player_commands, ai_key=None):
    """Advance the race by one lap.

    ``ai_key`` (optional) selects the LLM decision engine for this lap; without
    it the historical heuristic engine is used. The key is never stored on the
    race state — only the engine *name* lands in ``race_state["ai_engine"]``.
    """
    ...
    if race_state.get("weather_grace_laps", 0) > 0:
        ...
        _apply_ai_strategy(race_state, events, rng, ai_key)
...
def _apply_ai_strategy(race_state, events, rng=None, ai_key=None):
    """Decide AI pit/tyre moves for this lap via the configured decision engine.

    The engine is resolved lazily (per lap) from the optional API key and its
    name is recorded on the race state so saves keep a plain
    ``"heuristic" | "api"`` string — never the key or an engine object. The
    engine applies its own decisions (pit_pending + event messages).
    """
    rng = rng or random
    from simulation.ai.factory import resolve_engine

    engine = resolve_engine(ai_key)
    race_state["ai_engine"] = engine.name
    engine.pit_plan(race_state, events, rng)
